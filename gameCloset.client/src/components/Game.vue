      ...
      <div class="flip-card-back elevation-3">
        ...
        <div class="row">
          <div class="col text-start mt-3 ms-3">
            ...
            <h5 v-else>
              <a data-bs-toggle="offcanvas" :href="'#gameDetails-' + game.id">See More Details</a>
            </h5>
          </div>
        </div>
        ...
        <div class="row" v-if="user.isAuthenticated">
          <div class="col mt-3" v-if="isSearchResult">
            <div v-if="!hasGame">
              <p class="text-end">
                <button v-if="route.name == 'Search' && !game.owned" @click="addToWishlist(game)"
                  class="btn btn-secondary selectable me-3 p-2 rounded" title="add to wishlist">
                  Add to Wishlist
                </button>
