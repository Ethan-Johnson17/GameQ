      ...
      <div class="col-md-10 card my-2 elevation-3 px-5 py-3">
        <div class="row justify-content-center">
          ...
          <div class="col-md-3 text-end p-1">
            <button v-if="!player" class="btn btn-secondary px-4" @click="joinGameNight(activeGameNight.pin)">
              Join
            </button>
            <button v-if="player" class="btn btn-danger px-4" @click="unattendGameNight(player.id)">
              Leave
            </button>
          </div>
        </div>
                  ...
                  <div class="dropdown input-group">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                      <li v-for="game in closetGames" :key="game.atlasGameId">
                        <div class="dropdown-item selectable" :class="
            ...
            <div class="row" v-for="g in gameQueue" :key="g.id">
              <div class="col">
                ...
                <div class="vote d-flex">
                  <i class="mdi mdi-thumb-up-outline text-secondary f-16 pt-0 selectable h-25 me-2" @click="vote(g.id)"
                    v-if="!g.votes.includes(account.id)"></i>
                  <i class="mdi mdi-thumb-up text-secondary f-16 pt-0 selectable h-25 me-2" @click="vote(g.id)"
                    v-if="g.votes.includes(account.id)"></i>
                  <p class="mb-0">{{ g.game?.name }}</p>
                  <i class="
                      mdi mdi-trash-can f-20
                      text-danger
                      ms-auto
                      selectable
                    " @click="removeGameQueue(g.id)" v-if="account.id === g.accountId"></i>
                </div>
              </div>
            </div>
          </div>
